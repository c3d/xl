...
operator_. It is an link:#infix[infix operator] with a
link:#pattern[pattern] on the left and an
link:#implementation[implementation] on the right. In other words,
its pattern is `Pattern is Implementation`, where `Pattern` is a program
pattern, like `X+Y`, and `Implementation` is an implementation for
that pattern, for example `Add X, Y`. This operator can also be read as
