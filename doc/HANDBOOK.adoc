...
. XL has a method to ((extend the language)) with any kind of feature, not
  just functions or data types, but also ((programming construct))s,
  ((optimization))s, ((domain-specific notation))s, and more. Actually, all this
...
programming that are built-in in other languages, well-known
((programming construct))s such as ((loop))s, ((test))s, and so on.
...
language, as in the following code that verifies the Syracuse
conjecture:
...
It has to be ((built-in). The spaceship operator has to be
...
to discuss the implications. By contrast, all it takes in XL to
implement `<=>` in a variant that always returns `-1`, `0` or `1` is
the following:
...
away an expression like `X*0`, `X*1` or `X+0`, whereas XL makes it
extremely easy:
...
brick in most ((programming language))s. In general, I/O operations are
...
language))s is much more than skin deep. The syntax of XL is actually one
