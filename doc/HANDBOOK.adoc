...
functioning releases like the XL version used in Tao3D in the past, XL
is being reworked, and the compiler in this repository is presently not
suitable for any serious programming. Examples given below may sometimes
simply not work. Take it as a painful reminder that the work is far from
finished, and, who knows, as an idea for a contribution. See
link:#history-of-xl[HISTORY] for how we came to the present mess. The
link:../README.md[README] will give you a quick overview of the
language.
...
Note that link:HANDBOOK_4-types.md[types] in XL indicate the shape of
parse trees. In other words, the `cartesian` type above will match any
parse tree that takes the shape of the word `cartesian` followed by two
numbers, like for example `cartesian(1,5)`.
...
link:HANDBOOK_2-evaluation.md#scoping[still based on pattern matching].
...
`[[false]]`, is called a link:HANDBOOK_2-evaluation.md#metabox[metabox].
...
form of
https://en.wikipedia.org/wiki/Multiply–accumulate_operation#Fused_multiply–add[fused
multiply-add], which has benefits that include performance and
precision. Yet C++ will not allow you to overload `X*Y+Z` to use this
...
Unlike the C++ `iostream` facility, the XL compiler will naturally emit
...
link:HISTORY.md#xl-gets-a-theoretical-foundation-concept-programming[concept
...
straightforward C++ code, and the link:../src/scanner.cpp[scanner]
...
precisely define link:HANDBOOK_2-evaluation.md[program evaluation] or to
...
`'ABC'` are valid text in XL. If the delimiter is needed in the text, it
can be obtained by doubling it. For example, `"He said ""Hello"""` is
text containing `He said "Hello"`.
...
Previous: link:HANDBOOK_0-introduction.md[Introduction] Next:
link:HANDBOOK_2-evaluation.md[Program evaluation]
...
link:HANDBOOK_3-types.md#lifetime[live], but invisible to the current
...
link:HANDBOOK_1-syntax.md#extending-the-syntax[spelling and precedence]
...
link:HANDBOOK_6-modules.md[chapter about modules].
. Identifying words that switch to a
link:HANDBOOK_1-syntax.md#child-syntax[child syntax]: symbols that
...
which are called link:HANDBOOK_3-types.md##type-annotations[_type
annotations_], or an infix assignment operator `:=` with a `:` type
annotation on the left, called a _variable initialization_. Definitions,
type annotations and variable initializations are collectively called
...
In the case of a link:HANDBOOK_4-compilation.md[compiled
implementation], the compiler will most likely assign machine locations
to each of the declarations. When the program runs, a constant like `pi`
or the definition of `circumference` may end up being represented as a
machine address, and a variable such as `Radius` may be represented as a
"stack location", i.e. a preallocated offset from the current stack
pointer, the corresponding memory location only containing the value,
i.e. the right-hand side of `:=`. Most of the
link:HANDBOOK_3-types.md[type analysis] can be performed at compile
...
link:HANDBOOK_3-types.md[type constraints] are known and respected. For
...
Bindings can be marked as link:HANDBOOK_3-types.md#mutability[mutable]
or constant. In this document, bindings made with `:=` are mutable,
while binding made with `is` are constant. Since by default, an `X : T`
...
declarations in the link:HANDBOOK_7-standard-library.md[standard
library], no declaration has a larger pattern like `X * Y * Z` that
could match the whole expression. However, there is a definition for a
...
NOTE: The link:HANDBOOK_7-standard-library.md[standard XL library] only
provides implicit conversions that do not cause data loss. On most
...
link:HANDBOOK_3-types.md[type] of the expression matches the type on the
right of the infix.
...
This case applies to sub-patterns, as was the case for `0! is 1` in the
link:HANDBOOK_0-introduction.md#factorial[definition of factorial]. It
also applies to top-level patterns, which is primarily useflu in
link:#scoping[maps]:
...
NOTE: Historically, the link:HISTORY.md#bootstrapping-xl[XL2]
...
link:HANDBOOK_1-syntax.md#tweak-1-expression-vs-statement[expressions
...
of _virtual function_ in C++, although the XL implementation is likely
to be quite different. The C++ approach only allows dynamic dispatch
along a single axis, based on the type of the object argument. C++ also
...
This forms the basis of link:HANDBOOK_3-types.md#creation[constructors]
and link:HANDBOOK_3-types.md#interface[tagged types]in XL.
...
the C++ implicit `this` pointer, with the added benefits that multiple
...
lookup] in C++, i.e. make it possible to access code in the caller’s
...
link:HANDBOOK_4-compilation.md#caller-lookup[seems possible].
...
rationale are detailed in link:HANDBOOK_3-types.md#ownership[the next
chapter]. In all cases, the previous value that was held in the left
operand is link:HANDBOOK_3-types.md#destruction[destroyed] by the
assignment.
...
more details link:HANDBOOK_3-types.md#binding[in the next chapter].
...
Error values have the link:HANDBOOK_3-types.md#errors[`error` type]. For
...
link:HANDBOOK_3-types.md#ownership[owning type] that dynamically
...
NOTE: *RATIONALE* In languages such as C++, some members of a class can be
...
link:HANDBOOK_1-syntax.md#the-xl-parse-tree[parse trees]. A type
...
determined by link:HANDBOOKE_2-evaluation.md#scoping[scoping].
...
link:HANDBOOK_4-compilation.md#lifetime[restrictions on access types],
...
link:HANDBOOKE_2-evaluation.md#scoping[scope]. As long as the named
...
link:HANDBOOKE_2-evaluation.md#scoping[scope], but the value of their
...
_constructors_ in C++. It makes it possible for programmers to provide
...
link:HANDBOOK_2-evaluation.md#super-lookup[super lookup].
...
which is called an
link:HANDBOOK_2-evaluation.md##assignments-and-moves[_assignment_].
...
link:#destruction[destruction]. In C++, the mechanism is called
...
link:HANDBOOK_2-evaluation.md#when[conditional pattern] as follows:
...
The
link:HANDBOOK_2-evaluation.md#interface-and-implementation[interface] of
a type can specifiy a link:HANDBOOK_2-evaluation.md#scoping[scope] for
...
link:HANDBOOK_2-evaluation.md#pattern-matching-scope-values[match a
scope]
...
The link:HANDBOOK_2-evaluation.md#assignments-and-moves[assignment
operator] is written `A := B` in XL. For compact types, this is normally
...
of a type variable should be treated like "template code" to use C++
...
But my heart was never with Java anymore than with C++, as evidenced by
...
plug-ins]. I suspect that attributes in C++11 are distant (and less
...
concepts bring a similar kind of validation to C++ templates.
...
http://mozart-dev.sourceforge.net/news.html#complex[70% faster than C++
...
self-compile?_“. When I answered that the compiler was written in C++,
...
The initial translator converts a simplified form of XL into C++ using a
...
are simply regular XL functions implemented in C++:
