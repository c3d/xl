...
<\body>
  ...
  ELIOT is a dynamic language based on meta-programming, i.e. programs that
  ...
  <\warning>
    ...
    implemented. The current ELIOT implementations are currently incomplete. An
  ...
  except for what it's missing: ELIOT makes little use of parentheses and other
  ...
  ELIOT programs look a little like pseudo-code, except that they can be
  compiled and run.
  ...
  program to deal with programs. In the case of ELIOT, meta-programming is the
  ...
  Conceptually, an ELIOT program is a transformation of ASTs following a number
  of tree rewrite rules. In other words, there is no real difference in ELIOT
  between meta-programming and normal program execution, as both are
  represented by tranformations on ASTs. The entire ELIOT semantics boils down
  ...
  Figure<nbsp><reference|if-then> illustrates the ELIOT <strong|>definition of
  ...
  Figure<nbsp><reference|factorial>. This shows how, in ELIOT, meta-programming
  ...
  <paragraph|Influence on ELIOT>ELIOT is the first programming language designed
  ...
  i.e. values which cannot be decomposed into smaller units from an ELIOT point
  of view. They are:
  ...
  normal ELIOT ensures that most text parses as one would expect from daily use
  ...
  <\itemize>
    ...
    performed<index|evaluation order>. ELIOT has two infix sequence operators,
  ...
  *>>>|<row|<cell|<verbatim|boolean>>|<cell|<verbatim|bool>>>>>>|Type
  ...
  <index|library>The ELIOT language is intentionally very simple, with a strong
  ...
  the library in ELIOT. Implementing basic amenities that way is an important
  ...
  This section decsribes the minimum list of operations available in any ELIOT
  ...
  ELIOT lists use comma-separated lists<subindex|list|comma-separated>, such as
  ...
  ELIOT modules make it possible to decompose a large ELIOT program in smaller
  units.
  ...
  <\itemize>
    <item>A file name provides a system-dependent file name for an ELIOT source
    file. By convention, ELIOT source file names end in <verbatim|.xl>. In
  ...
  *Y)>|Signature for rewrite code with two variables.><label|closure-code>
  ...
  <\table-of-contents|toc>
    ...
    <with|par-left|6fn|Influence on ELIOT <datoms|<macro|x|<repeat|<arg|x>|<with|font-series|medium|<with|font-size|1|<space|0.2fn>.<space|0.2fn>>>>>|<htab|5mm>>
