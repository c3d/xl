...
#define AS_INT(x)       (new Integer((x), DataSelf(data)->Position()))
#define AS_REAL(x)      (new Real((x), DataSelf(data)->Position()))
#define AS_BOOL(x)      ((x) ? xl_true : xl_false)
#define AS_TEXT(x)      (new Text(x, DataSelf(data)->Position()))
...
/* ------------------------------------------------------------ */      \
    ...
    {                                                                   \
        ...
        {                                                               \
            Tree *self = xl_nil;                                        \
            Opcode_FC_##FName &_XLparms = *this;                        \
            _XLparms.size = 0;                                          \
            Parms;                                                      \
            if (result)                                                 \
        ...
        virtual Op *Run(Data data)                                      \
        {                                                               \
            FunctionArguments _XLparms(data, size);                     \
            Parms;                                                      \
            if (_XLparms.size != size)                                  \
