...
module XL_BUILTINS with
    ...
    replace (assert 'cond') with
        xl_assert_implementation({evaluate} 'cond'), {text} 'cond', 'cond')

    // Check the constant cases for static asserts
    replace (xl_assert_implementation(true, 'message', 'source')) with
        @nop

    replace (xl_assert_implementation(false, 'message', 'source')) with
        {error} "Assertion '$1' is false at compile time", 'source'

    // Otherwise, expand to a runtime test
    replace (xl_assert_implementation('cond', 'msg', 'src')) with
        if not ('cond') then
            xl_assertion_failure ('msg', {file} 'src', {line} 'src')
