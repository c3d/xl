...
module XL.POINTER with
    ...
    function Dereference (T : pointer) return pointer.T is XL.BYTECODE.deref_ptr
    function Pointer() return pointer is XL.BYTECODE.zero_ptr
    function Pointer(item : pointer.T) return pointer is XL.BYTECODE.new_ptr
    ...
    to Delete(out item : pointer.T) is XL.BYTECODE.delete_ptr
