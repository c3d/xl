...
scope may be provided in a different order, and the scope does not
need to use the same delimiters or separators:
...
....
to Resize (P:inout packet, S:size) is
...
....
to Resize (P:inout packet, S:size) when S = P.header.byte_count is nil
...
* `in_out T` can be used for input-output arguments of type `T`,
  ...
  ignores the `_` character, it is also possible to spell this type as
  `inout T`. Since this is very common, an alternate spelling, `io T`,
...
which may fail. Returning an `error` is not allowed by default by the
signature of `create` for efficiency reasons. If this is desirable,
then you must explicitly mark `create` as returning `mayfail`.
...
....
module COMPLEX[real:some number] with
