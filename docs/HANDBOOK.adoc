...
types (primarily intended for use with fixed-point subtypes) to
select a "saturating" behavior in case of overflow.
...
the syntax `type T with I`, where `I` is a scope containing the
publicly available declarations. These declarations are called
_fields_ of the type when they denote link:#mutability[mutable]
values, and _members_ of the type if they are constant. Globally, they
are called the _features_ of the type.
