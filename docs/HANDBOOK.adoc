...
be implemented as a structure that derives from `struct XL(type)`.
...
struct XL(shape)_interface : XL(type)
{
    void                (*XL(draw))(XL(shape) *shape);
    XL(coordinate)      (*XL(width))(XL(shape) *shape);
    XL(coordinate)      (*XL(width))(XL(shape) *shape, XL(coordinate) width);
}
....
...
very similar to so-called _vtables_ in pass:[C++].
