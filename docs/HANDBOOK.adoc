...
_assignment_ or a definition with the same name:
...
The second example above matches a link:#scope-pattern-matching[scope],
which means that the arguments may be given in a different order.
...
C3 : circle := circle <4>
    ...
    CenterY is 2.4
----
<1> Notice that the formal parameters are separated by semi-colons,
    which means that the block is a scope
...
<3> An argument scope is passed, which contains the necessary
    ...
    be in the same order as in the parameter scope.
