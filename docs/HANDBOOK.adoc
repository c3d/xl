...
When a block in a pattern defines a link:#scoping[_((parameter scope))_],
i.e. a link:#sequences[sequence] of declarations or definitions, it
can be matched by any scope that provides matching definitions. In
that case, the definitions in the _((argument scope))_ may be provided
in a different order, or with different separators for the scope:
...
C3 : circle := circle <4>
    ...
    CenterY is 2.4
----
<1> Notice that the formal parameters are a comma-separated sequence of
...
<3> An argument scope is passed, which contains the necessary
    definitions to match the parameter scope. The definitons need not
    be in the same order as in the parameter scope. However, passing
    them with the comma operator is inconvenient, because `,` is
    higher precedence than `is`.
<4> The argument scope need not use the same separators as the
    parameter scope. Using indentation and line separators is more
    convenient, since it removes the need for parentheses.
...
optimizations (most of which are obsolete or irrelevant today when
link-time optimizations are widely available), but also
...
possible to write _((constrained generic code))_, i.e. code that
...
For example, all array types share an internal organzation and provide
...
* `list` to store a variable number of linked elements. Unlike arrays,
  elements in lists are individually allocated in memory. Lists exist
  in several flavors:
...
typically creates a _((true generic type))_, which is a type that will
...
* `matrix` is a generic representation of mathematical
  ...
  with two underlying representations, `sparse` and `dense`. It
  features matrix algebra, and is intended to be used with `vector`.
  Operations on the `matrix` type are often highly parallelizable.
  The `matrix[4,3] of integer32` type will
...
The link:#interface-and-implementation[interface] of a type can
specifiy a link:#scoping[scope] for values that match the type, using
the infix form `[[type]] with Interface`, where `Interface` is an
_((interface scope))_ containing the publicly available
declarations. These declarations are called _features_ of the
type. The terms _fields_ of the type are also used for
link:#mutability[mutable] values, and _methods_ of the type for
constant values.
...
link:#true-generic-types[true generic types]. For concrete types,
however, the interface of a function making it possible to create
values will also be provided. In the rest of the discussion for the
...
totally_abstract as type       // All we know is that the type exists
----
...
no implementation, like `picture` above, is called a _((tag type))_.
...
For better readability, the pattern can also match a scope with
link:#scope-pattern-matching[matching definitions]:
...
Note that in order to make patterns more specific, a type is typically
defined using a prefix, not just a block, in order to define a
link:#creation[constructor]. For example, it would be
...
In general, you want the pattern for a type to be more specific, so it
is customary to add a prefix that matches the type name, therefore
creating a link:#creation[constructor].
