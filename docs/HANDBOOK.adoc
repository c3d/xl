...
[source]
----
write X:text            as ok   is ... // <1>
write X:integer         as ok   is ...
write X:real            as ok   is ...
write X:character       as ok   is ...
write [[true]]          as ok   is { write "true"  } // <2>
write [[false]]         as ok   is { write "false" }
write Head, Rest        as ok   is { write Head; write Rest }

print                   as ok   is { write SOME_NEWLINE_CHARACTER }
print Items             as ok   is { write Items; print }
----

<1> The link:#fallible-types[ok] type is used to represent
...
* link:#assignment-and-move[Assignment and move] operations are somewhat
  special insofar as they are used during parameter passing while
...
Assignments can either return the target of the assignment, or
`nil`. The value-returning variant can be used in expression, whereas the
`nil`-returning variant is used for statements. This is a result of
normal overloading, combined with the fact that statements expect a
`nil` value. One is normally implemented as the other, although the
choice of which form is primary is left as a choice for the
implementor.

For example, if for type `value`, the `nil`-returning assignment
operator is primary, the two operators can be defined as follows:

[source]
----
to Copy(Target: out value,
        Source: in value) as nil written Target := Source is ...
Target:out value := Source: in value as value is
    Copy Target, Source
    Target
----

If an error can be returned from the copy operation, then the correct
code would look like the following:

[source]
----
to Copy(Target: out value,
        Source: in value) as ok written Target := Source is ...
Target:out value := Source: in value as value? is
    Copy Target, Source
    Target
----
...
safe way remains to be validated. Also, the syntax may change, as it's unclear
that the proposed syntax is not ambiguous or unintuitive.
...
* `new T` is a type that inherits all operations from `T`, but is not
  directly compatible with `T`. Explicit conversions are required to
  copy or move a value from one type to another.
...
_((destructor))_ for type `T`. It often has an link:#in out[in out] parameter
...
which is called an
link:##assignment-and-move[_assignment_].
...
....
Target:out T :+ Source:T as T?
copy Source:T as T? is result :+ Source
....
...
....
copy (Source:T, StopConditions) as T?
....
...
....
copy(Source:tree, Depth:count) as T?
copy(Source:tree, Keep:node_filter) as T?
...
....
Target:out T :< Source:in out T as T?
move Source:T as T? is result :< Source
....
