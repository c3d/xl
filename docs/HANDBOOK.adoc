...
link:#my_file[[`MY_FILE`]] example, we defined `file` as an abstract
type:
...
`P.width` is known to be valid through the interface, and evaluating
this expression in the context of the pattern matching scope finds a
definition that matches what the interface specifies. The expression
is therefore directly available from the type implementation.
