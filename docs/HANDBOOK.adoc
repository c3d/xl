...
. [[symbols]]`symbols` nodes regroup names like `JOHN_DOE` and symbols like `+=`.
...
link:#extending-the-syntax[extend the syntax].
...
link:{xl}src/xl.syntax[syntax file], modules can also supply a `.syntax` file
...
write a `spaceship` module with a `spaceship.syntax` file:
...
. Processing `use` statements: Since imported modules can contain
  link:#extending-the-syntax[syntax extensions], they must at least partially be
...
value that is not mutable is said to be _constant_.
...
to Swap(inout X:array, inout Y:array) is
    for I in array.index_range loop
        Swap X[I], Y[I]
----
