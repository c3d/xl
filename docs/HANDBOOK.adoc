...
link:#access[access type] that is cheap to copy around, and that
...
for N:name in R:[range of discrete] loop Body is
    loop_context is
        [[N]] : R.type := R.first
    Var is loop_context.[[N]]
    while Var <= R.last loop
        (loop_context) (Body)
        Var := next_after Var
....

We need to create a `loop_context` scope that holds the variable
