...
* `in_out T` can be used for input-output arguments of type `T`,
  ...
  also possible to spell this type as `inout T`.
...
_Creation_(((creation))) is the process of preparing a value for
use. The XL language rules ensures that specific rules are followed to
invoke creation code provided by the programmer before any other
possible use of the value being created.
