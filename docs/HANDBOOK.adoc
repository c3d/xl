...
It is fundamental to realize that the XL type system provides very
strong guarantees even for data types as complicated as this one.
For example, the following code will fail type system checks:
...
<1> This is a type error on the `packet` type, because the existing
    ...
    result does not belong to the `packet` type.
...
resize (P:inout packet, S:size) when S>P.header.byte_count is
    ...
    new_payload : payload(S) := array
        \I when I < P.byte_count   is P.payload[I]
        \I                         is byte(0)
    P := packet(new_header, new_payload)
....
...
including link:#gneneric-containers[generic container types],
