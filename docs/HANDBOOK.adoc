...
Additional sizes may be added if they are native to the target
machine. For example, some DSPs feature 24-bit operations, and such
machines should feature types like `unsigned24` to match.
...
link:#subtypes[integer subtypes] to identify precise ranges of values,
as in `integer range 1..5`, which only accepts values between `1` and
`5`, or precise number of bits, such as `integer bits 24`, which
wraps around like a 24-bit `integer` value.

==== Category types

Some types are intended primarily as an easy way to categorize
values along genereally useful boundaries, and are naturally called
...
* `array` store a fixed number of consecutive elements. They exist
  ...
  ** Range-indexed arrays, such as `array['A'..'Z'] of boolean`, which
     are indexed with a`link:#discrete[discrete]` range of values
  ** Multi-dimensional arrays such as `array[1..8, 'A'..'H'] of chess_piece`
...
* `range of T` holds ranges of values of `T` type. For example, the
...
* `in T` can be used to pass input arguments of type `T`, i.e. values
  that are initialized and owned by the caller.

* `out T` can be used for output arguments of type `T`, i.e. values
  that are initialized by the callee.

* `inout T` can be used for input-output arguments of type `T`,
  i.e. values that can be modified by the callee in a way that
  respects ownership in the caller.
