...
In particular, the `ignorable T` (((ignorable type))) generic type provided by
the standard library can implicitly convert to either `T` or `nil` so that it
can be either returned from a function or used in a statement. Its definition is
roughly as follows:
...
link:#assignments[assignments], which are used primarily for the side effect of
