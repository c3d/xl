...
Selecting between the attribute notation or the assignment notation
for individual fields is largely a matter of taste. It is even
possible to provide both.

That choice is largely hidden from the interface, however, because
when attempting to find the implementation for an interface, the
compiler will transparently look for an attribute pair as a possible
implementation.
...
picture is type picture
    ...
    // Working case
    width  W:u16 is bits.width W
    height H:u16 is bits.height H
...
picture as type with
    width  as size
    height as size
    width W as mayfail size
    height H as mayfail size
    pixels : buffer[area] of byte
    area as size
----
...
Attributes are a convention to special-case the transfer of values to
a specific named entity, notably when it is important to control how
that value is written to or read from.

For example, a `background` feature with the `color` type can be
implemented as a variable:

....
background : color              // Variable holding the color
if background = blue then
    background := red           // Assigment style
....

Alternatively, the same feature can be provided as an attribute, which
may sometimes be more readable:

....
background as color             // Get the background color
background C:color as color     // Set the background color
if background = blue then
    background red              // Attribute-setting style
....

This is largely a matter of taste, and the preferred form depends on
the context. In addition, if a type interface exposes a feature `F`
and if there is an attribute named `F` in the implementation, then the
attribute will automatically be used to implement the feature, which
means that an operation like `X.F := V` may silently be transformed
into `X.F V` if necessary.

[NOTE]
=====
It is considered extremely bad taste in XL to use `get`
or `set` in the name of attributes. For example, the following code
is roughly 16 characters too long:
....
...
declarations can then be used to build _((type expressions))_.

For example, the following code defines a generic `complex` type to
...
Type expressions play for XL the role that "class templates" play in
pass:[C++], or "generic types" in Ada, although the implementation may
in some cases need to fallback to more dynamically typed ways to
evaluate the code.

By convention, type expressions use square brackets, as in
`complex[real]`, reserving parentheses for normal expressions, as in
`complex(1.2,3.4)`. There is no language requirement enforcing this,
however. It is perfectly legal, if unusual, to write `complex real` and
`complex[1.2,3.4]`.
...
* `T1 and T2` is a type for values that belong to both `T1` and `T2`.
  ...
  `totally_ordered`).
* `not T` is a type for values that do not belong to `T`.
...
can be changed using the `:=` operator or their variants.
+
[NOTE]
=====
By default, formal parameters are mutable, since > they are generally
specified with something like `X:integer`, but > modifications apply
to the binding in the current evaluation > context, therefore not
modifying the corresponding argument.
=====
