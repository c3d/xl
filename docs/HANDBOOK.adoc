...
The notation `DerivedType like BaseType` can also be used in the
link:#interface[interface] for a type. In both cases, the notation is
a promise that the derived type can be implicitly converted to the
base type. It does not specify how this is done. One way, indicated
