...
When a block in a pattern defines a link:#scoping[_((parameter
scope))_], i.e. a link:#sequences[sequence] of declarations or
definitions, it can be matched by any scope that provides matching
definitions. In that case, the definitions in the _((argument scope))_
may be provided in a different order, or with different separators for
the scope:
...
link:#pattern-matching-definitions[matching definitions]:
