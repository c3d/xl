...
This is not a purely theoretical concern. In pass:[C++], if the base
...
plays the role to _((virtual functions))_ in pass:[C++].
...
In the C code, the notation `XL(expr)` will denote a magical macro
...
In addition, the example code will borrow one feature from C++,
inheritance, so that we may write something like the following in the
examples:
...
representation that looks something like this, where `P` is a
shortcut for `XL.PARSER`:

....
enum kind { NATURAL, REAL, CHARACTER, ... };
struct XL(P.tree)                       { enum kind kind; position_t position; ... };
struct XL(P.natural)    : XL(P.tree)    { XL(natural) value; };
struct XL(P.real)       : XL(P.tree)    { XL(real) value; }
struct XL(P.character)  : XL(P.tree)    { XL(character) value; };
struct XL(P.text)       : XL(P.tree)    { XL(text) value; }
struct XL(P.bits)       : XL(P.tree)    { size_t size; void *data; }
struct XL(P.symbol)     : XL(P.tree)    { XL(text) value; }
struct XL(P.name)       : XL(P.symbol)  { };
struct XL(P.operator)   : XL(P.symbol)  { };

struct XL(P.prefix)     : XL(P.tree)    { XL(P.tree) *left, *right; };
struct XL(P.postfix)    : XL(P.tree)    { XL(P.tree) *left, *right; };
struct XL(P.infix)      : XL(P.tree)    { XL(P.tree) *left, *right;
                                          XL(text) name; }
struct XL(P.block)      : XL(P.tree)    { XL(P.tree) *child;
                                          XL(text) opening, closing; }
....
...
....
// complex is type complex(Re:real, Im:real)
typedef struct XL(complex) { XL(real) Re; XL(real) Im; } complex_t;

// Z : complex := complex(1.2, 3.4)
complex_t Z = { 1.2, 3.4 };

// Z1:complex + Z2.complex is complex(Z1.Re+Z2.Re, Z1.Im+Z2.Im)
#define complex_add  XL(Z1:complex + Z2.complex)
...
In the special case of types that contain only a machine type, that
...
typedef XL(real) distance_t;    // Probably a double

// D1:distance + D2:distance is distance(D1.meters + D2.meters)
#define distance_add XL(D1:distance + D2:distance)
...
representation for types with open-ended values will typically contain
the normal form, knowing that the passed normal form will generally be
the normal form for link:#closures[closures]:

....
// if_statement is type { if Condition then TrueClause else FalseClause }
struct XL(if_statement)
{
    XL(P.tree) *Condition;
    XL(P.tree) *TrueClause;
    XL(P.tree) *FalseClause;
};
....
...
....
typedef XL(type of expression) expression_type;
struct XL(expression)
{
    expression_type (*code) (struct XL(expression) *closure, args...);
    XL(P.tree) *self; // If needed
...
....
// adder N is { lambda X is X + N }
typedef struct XL(lambda X)
{
    int (*code) (struct XL(lambda X) *closure, int X);
} lambda;

typedef struct XL(lambda X capturing N) : lambda
{
    int N;
} lambda_N;

#define anonymous_function XL(X + N)
int anonymous_function(lambda_N *closure, int X)
{
    return X + closure->N;
}

#define adder_name      XL(adder N)
...
Actual data types are

A data type in XL has a dynamic representation that gives access to a
number of features of the type for
