...
====
[[bits]]
The text nodes are used for binary data such as:
...
This generates text nodes in the parse tree that have empty delimiters
(i.e. their delimiter is neither `"` nor `'`), and in the XL type system, this
corresponds to the `binary` data type.
====
