...
`mayfail T` contains four accessible fields:
...
as building blocks for a parse tree.

==== Basic data types

XL offers a number of basic data types that are available on all
implementations, without any `use` statement. They includes:
...
* `unsigned` is an approximation of unsigned numbers, with a limited
  ...
  whole number literal values such as `0` or `42`.
...
The XL standard library features a large number of _((generic types))_,
i.e. types that have parameters and are _instantiated_ with arguments
to create the desired types. Besides
link:#generic-containers[generic containers], which are discussed
below, this includes a number of useful types:

* `range` of values, defined in `XL.DATA.RANGE



=== Generic containers

An interesteding subset of generic types are _((container types))_,
i.e. types that are primarily designed to store a possibly large
number of values from some other type. The corresponding modules are
submodules of the `XL.DATA` module.

Container types include in particular:

* `array`, storing a fixed number of consecutive elements, defined in
  module `XL.DATA.ARRAY`. They exist in multiple flavors:
  ** Zero-based arrays, such as `array[5] of integer`.
  ** Range-indexed arrays, such as `array['A'..'Z'] of boolean`.
  ** Multi-dimensional arrays, such as `array[5,5] of character`.
+
....
A : array[1..5, 'A'..'Z', 8] of real
A[3, 'C', 2] := 3.3
....

* `string`, storing a variable number of consecutive elements, defined
  in module `XL.DATA.STRING`. The `text` type exposes a `string of
  character` interface. They also exist in multiple flavors:
  ...
  ** Multi-dimesional strings, such as `string[5,1..5] of boolean`.
+
....
S : string of integer
for I in 1..100 loop
    S := S & I
if not (S contains 4) then
    error "There is a problem with your string implementation"
....

* `list`, storing a variable number of linked elements, defined in
  module `XL.DATA.LIST`. Unlike arrays, elements in lists are
  individually allocated in memory. They exist in several flavors:
  ** Single-linked lists such as `list of integer`.
  ** Doubly-linked lists, called double-ended queue, such as `queue of integer`.
...
for I in 1..100 loop
    S &= I
if not (S contains 3) then
    error "There is a problem with your list implementation"
....

* `stack`, exposing `push` and `pop` operations, and the type is matched
...
for I in 1..100 loop
    push S, I
if pop S <> 100 then
    error "There is a problem with your stack implementation"
....

* `map`, offering efficient ways to associate (map) an index value to
  a contained value. For example, `map[text] of real` creates a map
  between `text` index values and `real` stored values:
+
....
M : map[text] of real
M["Hello"] := 3.14
...
More interesting types can be used using the `type` function, which
