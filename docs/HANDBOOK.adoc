...
accept any variant of the generic type. For example, for `array`, one
could write the true generic type as follows:

....
array is type (array[index:array_index] of value:type)
....
...
....
complex is type (complex[real:type like number])
....
