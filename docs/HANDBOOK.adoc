...
XL uses the `Target :+ Source` as a notation for a copy, the `+` in
that operator being a reminder that a new copy is created.

The `:+` operator has the following mandatory interface for all types:
...
`error` value if there was some failure.
...
....
copy Target:out T from Source:T where Condition as mayfail T
....
...
....
copy Target:out tree from Source:tree where Condition:node_condition as mayfail T
node_condition is type(N:node as boolean)
....
