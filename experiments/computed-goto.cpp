...
#define dprintf(...)
#define ADDR(x)         &&op_##x
#define NEXT            goto *(label[code[pc++]])
...
data Bytecode::Run(Stack &stack)
{
    ...
    void *label[] =
    {
        &&op_loadx, &&op_storex, &&op_loady, &&op_storey, &&op_alloc,
        &&op_copy, &&op_add, &&op_add_load, &&op_sub, &&op_sub_cst, &&op_sub_load_cst,
        &&op_cstx, &&op_csty, &&op_jne_cst, &&op_jump,
        &&op_call, &&op_call1x, &&op_ret, &&op_ret_cst
    };
