...
//
//     Garbage collectio in ELIOT is based on reference counting.
...
/* ------------------------------------------------------------ */      \
    void *operator new(size_t size)                                     \
    {                                                                   \
        return ELIOT::Allocator<type>::Allocate(size);                  \
    }                                                                   \
                                                                        \
    void operator delete(void *ptr)                                     \
    {                                                                   \
        ELIOT::Allocator<type>::Delete((type *) ptr);                   \
    }
...
// ----------------------------------------------------------------------------
{
    ...
    ELIOT_ASSERT(result && result->gc == GarbageCollector::GC());
    return result;
}
...
// ----------------------------------------------------------------------------
{
    ...
    {
        ELIOT_ASSERT (((intptr_t) pointer & CHUNKALIGN_MASK) == 0);
        ELIOT_ASSERT (IsAllocated(pointer));
...
// ----------------------------------------------------------------------------
{
    ELIOT_ASSERT(size == allocator->TypeAllocator::objectSize); (void) size;
