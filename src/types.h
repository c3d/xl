...
public:
    Types(Scope *scope, Tree *source);
    Types(Scope *scope, Tree *source, Types *parent);
    virtual ~Types();
...
public:
    // Virtual functions that are overriden by CompilerTypes at -O3
    virtual RewriteCalls *      NewRewriteCalls();
    virtual RewriteCandidate *  NewRewriteCandidate(Rewrite *, Scope *);
    virtual Types *             DerivedTypesInScope(Scope *scope, Tree *source);
    virtual RewriteCalls *      RewriteCallsFor(Tree *what);
