...
// ----------------------------------------------------------------------------
{
    Bytecode(Scope *scope, Tree *self, Tree *pattern, Parameters &parameters):
        code(), scope(scope), self(self), pattern(pattern),
...
public:
    ...
    void        NativeArguments(opcode_t nat, const Parameters &, opcode_t tgt);
    void        BuiltinArguments(const Parameters &parms, opcode_t tgt);
...
private:
    ...
    Tree *      pattern;        // Associated pattern
...
void Bytecode::BuiltinArguments(const Parameters &args, opcode_t target)
// ----------------------------------------------------------------------------
//   Push arguments for a native function
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
//   Return the local storage index for a given value (always writable)
// ----------------------------------------------------------------------------
{
    opcode_t index;
    ValueMap &values = compile->values;
    ...
    if (found != values.end())
    {
        ...
        // If what we have so far is a constant, need a writable copy
...
// ----------------------------------------------------------------------------
//   Reset the types to what is in the parameter list
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
{
    ...
    if (Name *name = IsLambda(what))
    {
        ...
        // Since [lambda X] is a catch all, we need to evaluate outside of it
        XL::Save<Scope_p> save(evaluation, evaluation);
        bool found = false;
        for (Scope *sc = evaluation; !found && sc; sc = sc->Enclosing())
            found = sc == declaration;
        if (found)
        {
            evaluation = declaration->Enclosing();
            arguments->Reparent(evaluation);
        }
        return name->Do(this);
    }
...
// ----------------------------------------------------------------------------
{
    ...
    if (!bytecode)
    {
        ...
        bytecode = new Bytecode(scope, expr, pattern, parameters);
