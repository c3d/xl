...
// ----------------------------------------------------------------------------
{
    ...
    if (function)
    {
        ELIOT_ASSERT(function->Inputs() == 0);
...
// ----------------------------------------------------------------------------
{
    ELIOT_ASSERT(count(instrs.begin(), instrs.end(), op) == 0);
    instrs.push_back(op);
    *lastOp = op;
    lastOp = &op->success;
    ELIOT_ASSERT(!op->success && "Adding an instruction that has kids");
}
...
// ----------------------------------------------------------------------------
{
    ELIOT_ASSERT(successOp && failOp);
    ELIOT_ASSERT(count(instrs.begin(), instrs.end(), failOp) == 0);
    ...
    *lastOp = successOp;
    ELIOT_ASSERT(failOp && "Success without a failure exit");
...
// ----------------------------------------------------------------------------
{
    ...
    if (isLeaf)
    {
        if (defined != eliot_self && !Tree::Equal(defined, self))
        {
            IFTRACE(compile)
                std::cerr << "COMPILE" << depth << ":" << cindex
                          << "(" << self << ") from constant "
...
// ----------------------------------------------------------------------------
{
    ...
    while (what)
    {
        ...
        if (candidates)
        {
            ...
            ELIOT_ASSERT(!*lastOp && "Built code that is not NULL-terminated");
    ...
    if (Name *name = self->AsName())
    {
        ...
        if (Tree *value = ctx->Bound(name, true, &rw, &scope))
        {
            ...
            case PARAMETER:
            {
                ...
                ELIOT_ASSERT(found != inputs.end() && "Parameter not bound?");
...
// ----------------------------------------------------------------------------
{
    ELIOT_ASSERT(inputs.find(name) == inputs.end() && "Binding name twice");
...
extern "C" void debugo(ELIOT::Op *op)
// ----------------------------------------------------------------------------
...
extern "C" void debugop(ELIOT::Op *op)
// ----------------------------------------------------------------------------
...
extern "C" void debugob(ELIOT::CodeBuilder *cb)
// ----------------------------------------------------------------------------
...
extern "C" void debugti(ELIOT::TreeIDs &tids)
// ----------------------------------------------------------------------------
//   Show the contents of a tree IDs map
// ----------------------------------------------------------------------------
{
    for (ELIOT::TreeIDs::iterator i = tids.begin(); i != tids.end(); i++)
